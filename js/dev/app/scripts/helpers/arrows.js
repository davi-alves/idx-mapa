define([], function () {
  ...
  ArrowHandler.prototype.draw = function () {
    ...
    if (this.arrowheads.length > 0) {
      var i, m;
      for (i = 0; i <= this.arrowheads.length; i++) {
        m = this.arrowheads[i];
        m.setOptions({
  ...
  ArrowHandler.prototype.create = function (p1, p2, mode, map) {
    ...
    else if (mode == "midline") markerpos = g.geometry.spherical.interpolate(p1, p2, 0.5);
  ...
  //- Sent the object to global scope
  window.ArrowHandler = ArrowHandler;


  // Draws a polyline with accordant arrow heads
  window.createPoly = function (path, mode, setArrows, map) {
